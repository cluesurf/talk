...
- **modifiers.json**: the affixes (secondary articulations, tone,
  ...
  spelling.
- **tokens.json**: the frozen, append-only list of `{ talk, token }`,
  one Hangul code point per canonical sound. Generated by
  [`../make/tokens.ts`](../make/tokens.ts). A code point is never
  renumbered once assigned, so tokens stay stable across releases.
- **clusters/**: per-category `{ talk, token }` lists (vowels and
  consonant onsets/codas/full/plain), one CJK code point per cluster,
  used by the syllabifier. Generated by
  [`../make/clusters.ts`](../make/clusters.ts).
