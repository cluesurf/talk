...
\cmt{# prajna, written in IPA}
talk.talk_to_ipa("pradjy~ny~a_") \cmt{# => "pradʑɲaː"}

\cmt{# one Hangul code point per sound}
talk.machine("pradjy~ny~a_") \cmt{# => a 7 code point string}

\cmt{# the sounds, each with its features}
talk.segment("pradjy~ny~a_") \cmt{# => [p, r, a, d, jy~, ny~, a_]}

\cmt{# and the reverse, IPA back to talk}
talk.ipa_to_talk("prɐdʑɲaː") \cmt{# => "pradjy~ny~a_"}
\end{Verbatim}
...
\item Conversion to and from IPA, and splitting of a word into syllables.
