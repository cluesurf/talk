...
const REPLACE: Record<string, string> = {
  ...
  //
  // Khoisanist work wrote the retroflex click `‼`, a doubled exclamation
  // mark, because no letter existed for it. Unicode encoded one in 2021 as
  // U+1DF0A, and the curated catalog spells it that way. phoible still
  // writes the old form, in !Xun and nowhere else, so its 12 spellings
  // matched no catalog entry and the seeder refused all of them.
  //
  // One sound, two spellings, which is what this table is for. Folding it
  // makes `k‼` reach the `k𝼊` the catalog already holds.
  '‼': '\u{1df0a}',
